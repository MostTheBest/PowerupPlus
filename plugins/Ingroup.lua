...
local function check_member_autorealm(cb_extra, success, result)
  ...
  for k,v in pairs(result.members) do
    ...
    if member_id ~= our_id then
      ...
      save_data(_config.moderation.data, data)
      return send_large_msg(receiver, '👁به قلمرو جدید خود خوش آمدید!🌟')
    end
  end
end
local function check_member_realm_add(cb_extra, success, result)
  ...
  for k,v in pairs(result.members) do
    ...
    if member_id ~= our_id then
      ...
      data[tostring(realms)][tostring(msg.to.id)] = msg.to.id
      save_data(_config.moderation.data, data)
      return send_large_msg(receiver, '👁قلمرو اضافه شده است!🌟')
    end
  end
end
function check_member_group(cb_extra, success, result)
  ...
  for k,v in pairs(result.members) do
    ...
    if member_id ~= our_id then
      ...
      save_data(_config.moderation.data, data)
      return send_large_msg(receiver, '🏵شما به عنوان مدیر گروه ترویج شده اید👁')
    end
  end
end
local function check_member_modadd(cb_extra, success, result)
  ...
  for k,v in pairs(result.members) do
    ...
    if member_id ~= our_id then
      ...
      data[tostring(groups)][tostring(msg.to.id)] = msg.to.id
      save_data(_config.moderation.data, data)
      return send_large_msg(receiver, '🏵گروه اضافه شده است و شما به عنوان صاحب ترویج شده است🌟')
    end
  end
end
...
local function lock_group_arabic(msg, data, target)
  ...
  if group_arabic_lock == 'yes' then
    return '🏵عربی در حال حاضر قفل شده است🔒'
  else
...
local function unlock_group_arabic(msg, data, target)
  ...
  if group_arabic_lock == 'no' then
    return '🏵زبان عربی در حال حاضر باز است🔓'
  else
    data[tostring(target)]['settings']['lock_arabic'] = 'no'
    save_data(_config.moderation.data, data)
    return '🏵عربی باز شده است🔓'
  end
end
...
local function lock_group_bots(msg, data, target)
  ...
  if group_bots_lock == 'yes' then
    return '🛡حفاظت رباتها در حال حاضر فعال است🤖'
  else
    data[tostring(target)]['settings']['lock_bots'] = 'yes'
    save_data(_config.moderation.data, data)
    return '🛡حفاظت رباتها فعال شده است🤖'
  end
end
...
local function unlock_group_bots(msg, data, target)
  ...
  if group_bots_lock == 'no' then
    return '🛡حفاظت رباتها در حال حاضر غیر فعال است🤖'
  else
    data[tostring(target)]['settings']['lock_bots'] = 'no'
    save_data(_config.moderation.data, data)
    return '🛡حفاظت رباتها غیر فعال است🤖'
  end
end
...
local function unlock_group_namemod(msg, data, target)
  ...
  if group_name_lock == 'no' then
    return '👥نام گروه را در حال حاضر باز است🔓'
  else
    data[tostring(target)]['settings']['lock_name'] = 'no'
    save_data(_config.moderation.data, data)
    return '👥نام گروه باز شده است🔓'
  end
end
...
local function unlock_group_floodmod(msg, data, target)
  ...
  if not is_owner(msg) then
    return "🌟فقط مالکان میتوانند سیل باز کنند🔓"
  end
  local group_flood_lock = data[tostring(target)]['settings']['flood']
  if group_flood_lock == 'no' then
    return '👥گروه سیل باز شده است🔓'
  else
...
local function modadd(msg)
  ...
  if is_group(msg) then
    return '👥گروه در حال حاضر اضافه شده است.👁'
  end
    receiver = get_receiver(msg)
...
local function realmadd(msg)
  ...
  if is_realm(msg) then
    return '🛡قلمرو در حال حاضر اضافه شده است.👁'
  end
    receiver = get_receiver(msg)
...
local function get_rules(msg, data)
  local data_cat = 'rules'
  if not data[tostring(msg.to.id)][data_cat] then
    return '👥در این گروه قانونی ثبت نشده است✍🏻'
  end
...
local function promote(receiver, member_username, member_id)
  ...
  if data[group]['moderators'][tostring(member_id)] then
    return send_large_msg(receiver, member_username..' در حال حاضر یک مدیر.👁')
  end
  data[group]['moderators'][tostring(member_id)] = member_username
  save_data(_config.moderation.data, data)
  return send_large_msg(receiver, member_username..' ترفیع گرفت.👁')
end
...
local function setowner_by_reply(extra, success, result)
  ...
  data[tostring(msg.to.id)]['set_owner'] = tostring(msg.from.id)
      ...
      local text = msg.from.print_name:gsub("_", " ").." صاحب اکنون👁"
...
local function modlist(msg)
  ...
  if next(data[tostring(msg.to.id)]['moderators']) == nil then --fix way
    return '👥در این گروه مدیری وجود ندارد🌟'
  end
  local i = 1
  local message = '\n👥فهرست مدیران برای گروه🌟 ' .. string.gsub(msg.to.print_name, '_', ' ') .. ':\n'
